...
def initiate():
    ...
    """
    bproc.renderer.set_render_devices(
        desired_gpu_device_type=["CUDA"], desired_gpu_ids=[0,1]
    )
...
def create_obj(blend_path, real_length):
    ...
    bproc_mats = []
    for mat in paint_mat:
        bproc_mat = bproc.material.convert_to_materials([mat.material])[0]
        bproc_mats.append(bproc_mat)

    paint.clear_materials()
    for mat in bproc_mats:
        if mat is None:
            continue
        else:
            paint.add_material(mat)
    
    # reassign material slots to faces
    bpy.context.view_layer.objects.active = paint.blender_obj
    paint.blender_obj.select_set(True)
    bpy.ops.object.mode_set(mode='EDIT')
    bpy.ops.mesh.select_all(action='DESELECT')
    bpy.context.tool_settings.mesh_select_mode = (False, False, True)  # face select mode
    # Need to switch to OBJECT mode in order to select face by index
    # since selections using indices on data.vertices only work in OBJECT mode
    bpy.ops.object.mode_set(mode = 'OBJECT')
    mesh = paint.blender_obj.data
    # select face on the left side mirror
    mesh.polygons[6155].select = True
    bpy.ops.object.mode_set(mode = 'EDIT')
    bpy.ops.mesh.select_linked()
    bpy.context.object.active_material_index = 2   
    bpy.ops.object.material_slot_assign() 

    # reassign black color to the right side mirror 
    bpy.ops.mesh.select_all(action='DESELECT')
    bpy.ops.object.mode_set(mode = 'OBJECT')
    mesh.polygons[2301].select = True
    bpy.ops.object.mode_set(mode = 'EDIT')
    bpy.ops.mesh.select_linked()
    bpy.context.object.active_material_index = 2   
    bpy.ops.object.material_slot_assign() 

    # set selection to whole paint again
    bpy.ops.object.mode_set(mode='OBJECT')
    bpy.context.view_layer.objects.active = bpy_obj
    bpy_obj.select_set(True)
    return root, paint, obj_reach
...
def find_island(islands, poly_idx):
    for island_index, island in enumerate(islands):
        for idx in island:
            if idx == poly_idx:
                if DEBUG:
                    print(f"Found idx {poly_idx} in island at index: {island_index}")
                return island_index
    return None  
...
obj, paint, obj_reach = create_obj(args.blend_path, args.length)
...
for run in range(args.runs):
    ...
    for i in range(args.frames):
        ...
        while fits == False:
            fits = True
            # select known poly
            if args.setup_type == "debug":
                poly = None
                for pol in selected_polys:
                    if pol.index == 27748:
                        poly = pol
            else:
            ...
            elif args.setup_type == "debug":
                cam_location = np.array([0.14, 0, 0.11])
